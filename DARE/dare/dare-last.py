...
def get_weight(new_w):
    ...
    pos=np.load('svhn_alexnet_slice_pos_18.npy')
    val=np.load('svhn_alexnet_slice_val_18.npy')
    rs = deepcopy(val)
    for i in range(3):
        for j in range(3):
            for k in range(3):
...
if __name__ == "__main__":
    ...
    rs=np.load('svhn_alexnet_18_rs.npy') 
    modify=np.load('svhn_alexnet_18_modify.npy') 
    ...
    new_model.save_weights('svhn_alexnet_fintune_last.h5')
