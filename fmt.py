...
def FMT(dataset, protected, method, learning_rate, batch_size, input_shape, repeat_index):
    """
    ...
    save_dir = os.path.join(os.getcwd(), "models", "fmt")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    new_model.save(
        "%s/fmt_%s_%s_%s_%s_%s_%s.h5" % (save_dir, method, dataset, protected, str(learning_rate), str(batch_size), repeat_index)
    )
...
def main(opt):
    ...
    pool.starmap(func=get_fairness, iterable=params)
    pool.close()
    pool.join()
