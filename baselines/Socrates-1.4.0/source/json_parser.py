...
def parse_dist(size, spec):
    dist = np.array(ast.literal_eval(read(spec)))

    mean = np.zeros(size)
    std = np.zeros(size)

    step = int(size / len(dist))

    for i in range(len(dist)):
        dist_i = dist[i]

        mean[i * step : (i + 1) * step] = dist_i[0]
        std[i * step : (i + 1) * step] = dist_i[1]
...
def parse_model(spec):
    ...
    if 'distribution' in spec:
        mean, std = parse_dist(np.prod(shape), spec['distribution'])

    return Model(shape, lower, upper, layers, path, mean, std)
