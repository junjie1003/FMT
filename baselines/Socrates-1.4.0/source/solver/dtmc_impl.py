...
class DTMCImpl():
    def __init__(self):
        ...
        self.dist = True    #turn on gaussian distribution
        self.sens_threshold = 0

    def __generate_x_random(self, shape, lower, upper):
        size = np.prod(shape)
    ...
    def gaussian_generate_x(self, shape, mu, std, lower, upper):
        size = np.prod(shape)
        # x = np.random.rand(size)
        x = []
    ...
    def __generate_x(self, shape, lower, upper, mean=None, std=None):
        if self.dist:
            return self.gaussian_generate_x(shape, mean, std, lower, upper);
        else:
            return  self.__generate_x_random(shape, lower, upper)
    ...
    def solve(self, model, assertion, display=None):
        ...
        pmc_time = time.time() - now_t
        print('pmc time:{}'.format(pmc_time))
        if is_fair:
    ...
    def _solve(self, model, assertion, display=None):
        overall_starttime = time.time()
        self.model = model
        self.assertion = assertion
        self.display = display
        ...
        for i in range (0, self.repair_num):
            # layer
            ran_layer_idx = int(np.random.rand() * (len(self.neurons) + 1))

            ran_neuron = 0
            ran_layer = ran_layer_idx
            if ran_layer_idx == 0:
                ran_neuron = int(np.random.rand() * len(other_features))
            else:
                ran_layer = self.intermediate_layers[ran_layer_idx - 1]
                ran_neuron = int(np.random.rand() * len(self.neurons[ran_layer_idx - 1]))
            self.repair_neuron.append(ran_neuron)
            self.repair_layer.append(ran_layer)

        self.repair_layer = [0, 1, 1, 1, 1, 0, 0, 3, 1, 9]
        self.repair_neuron = [7, 16, 50, 8, 63, 1, 4, 20, 15, 0]
        '''
        self.repair_num = 0
        for i in range (0, len(self.intermediate_layers) + 1):
            if i == 0:
                for j in other_features:
                    self.repair_neuron.append(j)
                    self.repair_layer.append(i)
                    self.repair_num = self.repair_num + 1
            else:
        ...
        repair_time = time.time() - analyze_time - preparation_time - overall_starttime
        ...
        valid_time = time.time() - repair_time - analyze_time - preparation_time - overall_starttime
    ...
    # API for causal solver to analyze fairness
    def analyze_fairness_causal(self, model, fairness_f, sens_cluster, timeout, criteria, error, confidence, datapath,
                                outpath, r_layer, r_neuron, r_weight, sens_threshold=0):
        overall_starttime = time.time()
        self.model = model
        self.dbgmsg = True

        self.sensitive = fairness_f
        self.sens_cluster = sens_cluster
        self.timeout = timeout
        self.criteria = criteria
        self.error = error / 2.0
        self.delta = 1 - math.sqrt(1 - confidence)
        ...
        print('\nAnalyze fairness using dtmc solver:')
        ...
        if len(r_layer) == 0:
            ...
            if is_fair:
                ...
                print('Network is unfair!')
            print('Maximum probability difference: {:.5f}'.format(prob_diff))


        else:

            # after repair
            self.repair_neuron = r_neuron
            self.repair_layer = r_layer
            ...
            # self.dbgmsg = True
            # verify prob diff and model accuracy after repair
            r_prob_diff, r_acc = self.test_repaired_net_causal(best_pos)
            print('Probability difference after repair: {}'.format(r_prob_diff))
            #print('Network Accuracy after repair: {}'.format(r_acc))
            ...
            if not path.exists(self.output_path + '/repair'):
                os.mkdir(self.output_path + '/repair')
            fout = open(self.output_path + '/repair/weight.txt', 'w+')
            for item in r_weights:
                fout.write(str(item.tolist()))
        ...
        # timing measurement
        print('DTMC solver execution time(s): {}\n'.format(time.time() - overall_starttime))
    ...
    def test_repaired_net_causal(self, weight):
    ...
    def get_new_sample_causal(self, k_sens, k_feature, k_hidden):
      #  print('model:',self.model)
       ...
       # print('lower:',lower)
        ...
        while generated:
            x = self.__generate_x(self.model.shape, lower, upper, self.model.mean, self.model.std)
            y = 0
        ...
        for i in range(0, self.step):
            path_ = [self.s0]
            to_add = (int(sens_array[i]) + 1) << self.bitshift[1]
            if len(self.feature) != 0:
    ...
    def learn_dtmc_model_causal(self):
        #file = open(self.gen_path, 'w+')
        ...
        self.init_model()
        while (self.is_more_sample_needed() == True):
       #     print('kmeans_sens:',kmeans_sens)
        #    print('kmeans_other:',kmeans_other)
         #   print('kmeans_hidden:',kmeans_hidden)
            path, kmeans_sens, kmeans_other, kmeans_hidden = self.get_new_sample_causal(kmeans_sens, kmeans_other, kmeans_hidden)
